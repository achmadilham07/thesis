//
//	Project ini hanya untuk simulasi dengan 4 kamera.
//	Posisi kamera adalah Cam1 (0,0) ; Cam2 (3.5,8) ; Cam3 (4.5,0) ; Cam4 (8,8)
//
...
vector<int> seqID = { 0,1,2,3 };
vector<double> posX = { 0, 3.5, 4.5, 8 }; //{ 6.7, 0.0, 0.0 };//{ 0.0, 6.7, 0.0 };
vector<double> posY = { 0, 8, 0, 8}; //{ 5.0, 0.0, 10.0 };//{ 0.0, 5.0, 10.0 };
vector<double> orientationCamera = { 30, 0, 0, 30 }; //{ 0, -30.0, 30.0 };//{ -30.0, 0, 30.0 };
vector<string> sideField = { "base", "upbase" , "base", "upbase" }; //{ "upbase" , "base", "base" }; //{ "base", "upbase" , "base" };
vector<string> axisField = { "horizontal","horizontal", "horizontal", "horizontal" };
	...
	for (int iteration = 0; iteration < iteration2.size(); iteration++) {
...

		{
			...
			case 0:
				...
				BBtime = getTickCount();
				diff_time = (BBtime - AAtime) / getTickFrequency();
				break;
			case 1:
				AAtime = getTickCount();
		...
		if (REPORT) {
			hog_kf2 << iteration << "\t" << newPosX << "\t" << newPosY << "\t" << id_client << "\t" << diff_time << endl;
		}
