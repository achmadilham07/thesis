...
int main(int argc, char** argv)
{
	const char* keys =
	{
		...
		"{ft    |     | File name for next position (.txt)}"
	};
	...
	String nextPos = parser.get< String >("ft");
	...
	while (checkClientConnect)
	{
		while (checkClientActive)
		{
			...
			if (true) {
				if (COLOR_DET) {
					...
					posX_hog.push_back(coord_x[iter]);
					posY_hog.push_back(coord_y[iter]);
			...
			if (KF2 && after_hog) {
				...
				if (afterHO) {
					kfPos2.updatePos2(newXkf, newXkf);
					afterHO = false;
				}
			...
			printf("\t%2.2f %2.2f \t", coord_x[iter], coord_y[iter]);
			...
			if (after_hog)
				...
				sprintf_s(namefile, "$%s_%06.2f_%06.2f#", IDClient.c_str(), posX_kf2[0], posY_kf2[0]);
			...
			iteration++;
			key = (char)waitKey(1);
			if (27 == key || iteration == iteration2.size()) {
				checkClientActive = false;
		...
		if (27 == key || iter == iteration2.size()) {
			break;
		}
...
bool splitBool(char* buf, int valread) {
	...
	while ((beg = text.find_first_not_of(delims, pos)) != std::string::npos)
	{
		...
		strSplit = text.substr(beg, pos - beg);
		arrText.push_back(strSplit);
	}

	int size = arrText.size();

	string IDClient = arrText[0];
	iter = stoi(arrText[1]);
	if (size > 3) {
		newXkf = stof(arrText[2]);
		newYkf = stof(arrText[3]);
		afterHO = true;
	}
	return stoi(arrText[size -1]);
}
